...
public class IEJoinBloomFinal {
	Vector<ResultantSet> rsVector ;

	 int index = 0;

	public void join(String relation1, String relation2, Integer op1, Integer op2, int operand1,
			int operand2, int operand3, int operand4) {
...
//		System.out.println("offsetArray2 " + Arrays.toString(offsetArray2));
...

		{
			for (int i = 0; i < m; i++) {
				...
				for (int k = off1 + eqOff; k < n; k++) {
					if (bitArrayD[k] == 1) {
						System.out.println(tuplesListL2.get(i).getAttr1() + "," + tuplesListL2.get(i).getAttr2()
								+ "----" + tuplesListL2D.get(k).getAttr1() + "," + tuplesListL2D.get(k).getAttr2());
					}
				}
		...
		{
			int[] bloomArrayD = new int[(bitArrayD.length / chunkSize)+1];
			...
			for (int i = 0; i < m; i++) {
				...
				for (int j = 0; j <= off2; j++) {
					...
					bitArrayD[permuteArrayPD[j]] = 1;
					int bloomPos = permuteArrayPD[j] / chunkSize;
					bloomArrayD[bloomPos] = 1;
				}

				int off1 = offsetArray1[permuteArrayP[i]];

				/*
				 * 1)Find the value in bloom Filter, here it's index is off1/4
				 * 2)Iterate towards right from the index value obtained in the
				 * above step
				 * 
				 */

				int bloomIndex = (off1) / chunkSize;

				for (; bloomIndex < bloomArrayD.length; bloomIndex++) {

					if (bloomArrayD[bloomIndex] == 1) {
						
						int bloomIndexEnd = (bloomIndex + 1)*chunkSize;
						if(tuples2.size() < bloomIndexEnd)
							bloomIndexEnd = tuples2.size();
						
						int bloomIndexStart = (bloomIndex)*chunkSize;
						if(off1 + eqOff > bloomIndexStart)
							bloomIndexStart = off1 + eqOff;
						
						
						for (int k = bloomIndexStart; k < bloomIndexEnd; k++) {
							if (bitArrayD[k] == 1) {
								count++;
								ResultantSet rs = new ResultantSet(tuplesListL2.get(i), tuplesListL2D.get(k));
								rsVector.add(rs);
								// System.out.println(tuplesListL2.get(i) +
								// "----" + tuplesListL2D.get(k));
							}
						}
					}
	...
	public static void main(String args[]) {
		...
		//
		// (id,age)
		// tuples1.add(new Relation(140, 9));
		// tuples1.add(new Relation(100, 12));
		// tuples1.add(new Relation(90, 5));
		//
		// // new Object(20,20);
		//
		// tuples2.add(new Relation(100, 6, 0, 0));
...
//		join(relation1, relation2, op1, op2, operand1, operand2, operand3, operand4);
